...
- Name: critical-flows.spec.ts >> Login Flow >> should show error with invalid credentials
- Location: e2e\critical-flows.spec.ts:25:7
...
Locator: locator('text=Error, text=Credenciales, text=Invalid, text=Incorrectas')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('text=Error, text=Credenciales, text=Invalid, text=Incorrectas')
...
- main:
  - text: psychology
  - heading "AulaIA" [level=1]
  - heading "Bienvenido nuevamente" [level=2]
  - paragraph: Ingrese sus credenciales para acceder al portal.
  ...
  - textbox "Contraseña":
    - /placeholder: ••••••••
    - text: wrong
  - button "Ver contraseña": visibility
  - checkbox "Recordarme"
  - text: Recordarme
...
> 30  |     await expect(page.locator('text=Error, text=Credenciales, text=Invalid, text=Incorrectas')).toBeVisible({ timeout: 5000 });
  ...
  123 |     await expect(page.locator('text=Escanea tu código QR')).toBeVisible();
  124 |   });
  125 | 
  126 |   test('should show manual code input option', async ({ page }) => {
  127 |     await page.goto('/modo-aula');
  128 |     await expect(page.locator('text=Ingresar código manual, button:has-text("Ingresar código manual")')).toBeVisible();
  129 |   });
  130 | });
```