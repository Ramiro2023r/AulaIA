...
class EstudianteTelegramDisabledControllerTest {
    ...
    @Test
    void telegramDeshabilitadoDevuelve409() throws Exception {
        mockMvc.perform(post("/api/v1/estudiantes/1/telegram/vinculacion")
                        .header(HttpHeaders.AUTHORIZATION, "Bearer " + adminToken))
                .andExpect(status().isConflict()) // 6. Telegram deshabilitado
                .andExpect(jsonPath("$.code").value("TELEGRAM_DISABLED"));
    }

    @Test
    void contextoArrancaCorrectamenteSinRepositorio() {
        // 9. contexto Spring arranca con TELEGRAM_ENABLED=false y no falla pidiendo TelegramVinculacionRepository
        // Si este test se ejecuta, significa que el contexto cargó exitosamente sin TelegramVinculacionRepository.
    }
}
